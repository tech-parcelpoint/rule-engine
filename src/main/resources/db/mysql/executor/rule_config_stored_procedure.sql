...
DELIMITER |
CREATE PROCEDURE execute_rule_engine(IN p_rule_module VARCHAR(50), IN p_entity_id VARCHAR(20), OUT p_rule_id INTEGER)
BEGIN
   ...
   ORDER BY rc.sort_order asc;
	...
	execute_rule: LOOP
		...
		SET @SQL = REPLACE(v_sql, 'from ', 'into @tmp_parcel_id from ');

		SET @SQL = CONCAT(@SQL, "'", p_entity_id, "'");
